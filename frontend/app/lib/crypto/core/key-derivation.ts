...
async function deriveKeyArgon2id(
  ...
  const combinedSalt = concatUint8Arrays([salt, userIdBytes]);
...
export async function encryptSeedForCloud(
  ...
  const keyMaterial = await deriveKeyArgon2id(password, salt, userId);
...
export async function decryptSeedFromCloud(
  ...
  try {
    ...
    // Derive key
    const keyMaterial = await deriveKeyArgon2id(
      password,
      salt,
      userId,
      encryptedData.kdfParams
    );
