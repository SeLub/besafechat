...
export class MediaController {
  ...
  @ApiOperation({ 
    ...
    description: 'Upload avatar image. Will be auto-resized to 256x256 PNG. Max size: 5MB. Supported formats: JPG, PNG, GIF, WebP.'
  })
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          type: 'object',
          properties: {
            url: { type: 'string', example: 'https://s3.tebi.io/besafe.backet/handles/d4461b295551e5da/avatar.png' }
          }
        }
      }
    }
  ...
  @ApiOperation({ 
    ...
    description: 'Upload image for chat messages. Max size: 10MB. Supported formats: JPG, PNG, GIF, WebP.'
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          type: 'object',
          properties: {
            url: { type: 'string', example: 'https://s3.tebi.io/besafe.backet/handles/d4461b295551e5da/media/images/1738095123456.jpg' }
          }
        }
      }
    }
  ...
  @ApiOperation({ 
    ...
    description: 'Upload document file. Max size: 50MB. Supported formats: PDF, DOC, DOCX, XLS, XLSX, TXT.'
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          type: 'object',
          properties: {
            url: { type: 'string', example: 'https://s3.tebi.io/besafe.backet/handles/d4461b295551e5da/media/documents/document.pdf' }
          }
        }
      }
    }
  ...
  async uploadDocument(@CurrentHandle() handle: any, @UploadedFile() file: MulterFile) {
    ...
    const url = await this.mediaService.uploadDocument(handle.id, file.buffer, file.originalname, file.mimetype);
    return new ApiResponseDto(true, { url });
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Upload audio file for voice messages. Max size: 20MB. Supported formats: MP3, WAV, OGG.'
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          type: 'object',
          properties: {
            url: { type: 'string', example: 'https://s3.tebi.io/besafe.backet/handles/d4461b295551e5da/media/audio/1738095123456_voice.mp3' }
          }
        }
      }
    }
  ...
  @ApiOperation({ 
    ...
    description: 'Upload encrypted seed phrase for cloud recovery. Max size: 1KB. Used for account recovery.'
  })
  @ApiConsumes('multipart/form-data')
  @ApiParam({ 
    name: 'passwordHash', 
  ...
  @ApiOperation({ 
    ...
    description: 'Delete a specific media file by type and path. Only files owned by current user can be deleted.'
  })
  @ApiParam({ 
    name: 'type', 
    enum: MediaType,
  ...
  async deleteFile(@Param('type') type: MediaType, @Param('filepath') filepath: string, @CurrentHandle() handle: any) {
