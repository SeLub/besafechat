...
- ✅ **Unified Storage API**: Единый `/media` endpoint для всех типов файлов (аватары, изображения, документы, аудио, видео)
...
- ✅ **S3 Storage Structure**: Хэшированные пути для приватности (`handles/{hash16(handleId)}/avatar.png`, `handles/{hash16(handleId)}/media/{type}/`)
- ✅ **Media Upload System**: Поддержка аватаров (PNG, до 5MB), изображений (до 10MB), документов (до 50MB), аудио (до 20MB), видео (до 25MB)
...
- ✅ **Медиа-система**: Единая система загрузки файлов с поддержкой аватаров, изображений, документов, аудио и видео

---

## 🗂 Структура S3-хранилища

BeSafeChat использует Tebi (S3-совместимое хранилище) для хранения медиафайлов. Структура хранения спроектирована для обеспечения приватности и масштабируемости:

```
s3://besafe.backet/
├── handles/
│   └── {hash16(handleId)}/           ← Хэшированный ID пользователя (16 символов)
│       ├── avatar.png                 ← Аватар пользователя (PNG, до 5MB)
│       ├── media/
│       │   ├── images/               ← Изображения из чатов
│       │   │   ├── {messageId}_photo1.jpg
│       │   │   └── {messageId}_photo2.png
│       │   ├── videos/               ← Видеофайлы из чатов (до 25MB)
│       │   │   ├── {messageId}_video1.mp4
│       │   │   └── {messageId}_video2.webm
│       │   ├── documents/            ← Документы из чатов (до 50MB)
│       │   │   ├── {messageId}_document.pdf
│       │   │   └── {messageId}_report.docx
│       │   └── audio/                ← Аудиофайлы из чатов (до 20MB)
│       │       └── {messageId}_voice.mp3
│       └── backups/
│           ├── messages_{timestamp}.db.enc
│           └── contacts_{timestamp}.json.enc
└── seeds/
    └── {hash(password)}/             ← Зашифрованные seed-фразы для восстановления
        └── seed.enc
```

### Особенности структуры:

1. **Хэшированные пути**: `handleId` преобразуется в 16-символьный хэш SHA256 для обеспечения приватности
2. **Прямые URL**: Аватары доступны по прямым ссылкам без необходимости получения presigned URL
3. **Автоматическое размещение**: Файлы автоматически размещаются в соответствующих подкаталогах
4. **Безопасность**: Доступ к медиафайлам контролируется через архитектуру приложения

---

## 📤 Система загрузки медиа

### Единая точка API

Все загрузки медиафайлов происходят через унифицированный API:

| Метод | Путь | Описание |
|-------|------|----------|
| `POST` | `/media/upload/avatar` | Загрузка аватара (PNG, до 5MB) |
| `POST` | `/media/upload/image` | Загрузка изображения (до 10MB) |
| `POST` | `/media/upload/document` | Загрузка документа (до 50MB) |
| `POST` | `/media/upload/audio` | Загрузка аудио (до 20MB) |
| `POST` | `/media/upload/video` | Загрузка видео (до 25MB) |
| `DELETE` | `/media/avatar` | Удаление аватара |
| `DELETE` | `/media/{type}/{path}` | Удаление других файлов |

### Поддерживаемые форматы:

- **Аватары**: PNG, JPG, GIF, WebP (автоматическое преобразование в PNG 256x256)
- **Изображения**: JPG, PNG, GIF, WebP (до 10MB)
- **Видео**: MP4, WebM, OGG, MOV (до 25MB)
- **Аудио**: MP3, WAV, OGG (до 20MB)
- **Документы**: PDF, DOC, DOCX, XLS, XLSX, TXT (до 50MB)

### Пример загрузки аватара:

```javascript
// Загрузка аватара через MediaService
const result = await MediaService.uploadAvatar(file);
const avatarUrl = result.url; // https://s3.tebi.io/besafe.backet/handles/{hash}/avatar.png
```

### Обработка изображений:

- Аватары автоматически преобразуются в формат PNG размером 256x256 пикселей
- Используется библиотека `sharp` для обработки изображений на сервере
- Применяется сжатие без потерь для оптимизации размера
